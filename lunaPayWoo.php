...
//             $this->icon = apply_filters('lpw_icon', plugins_url('assets/lunaPa.png', __FILE__));
            $this->icon = apply_filters('lpw_icon','https://lunapay.sgp1.digitaloceanspaces.com/iconlogo.png');
            ...
            if (isset($this->live) && $this->live === 'yes') {
                $this->current_url = 'https://app.lunapay.com'; 
            }else{
...
//              exit();
            ...
            if($status === 'Paid'){
             ...
             $order->set_transaction_id( $data['lunapay_trx_no'] );
             $order->save();

             $order->add_order_note('Payment Status: PAID' . '<br>Payment ID: ' . $data['payment_id']. '<br>Transaction No: '. $data['lunapay_trx_no']);
